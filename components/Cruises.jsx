...
const cruiseStyles = {
  ...
  ovl: {
    ...
    inset: 0,
    background:
      'linear-gradient(to top, rgba(10,22,40,0.92) 0%, rgba(10,22,40,0.2) 55%, rgba(10,22,40,0.05) 100%)',
  },
  ovlLight: {
    position: 'absolute',
    inset: 0,
    background:
      'linear-gradient(to top, rgba(10,22,40,0.75) 0%, rgba(10,22,40,0.1) 60%)',
  },
...
const CRUISES = [
  {
    ...
    t: 'Luxury whale',
    em: 'watching',
    body: 'Meet the humpbacks from the foredeck of Sun Goddess. Breach season May through October.',
    dur: '4 hrs',
    vessel: 'Sun Goddess',
    lg: true,
    col: 'span 7',
...
const CruiseCard = ({ c }) => {
  ...
  return (
    <div
      style={{ ...cruiseStyles.card, gridColumn: c.col, gridRow: c.row }}
      onMouseEnter={() => setH(true)}
      onMouseLeave={() => setH(false)}
    >
      <div
        style={{
          ...cruiseStyles.img(c.img),
          transform: h ? 'scale(1.06)' : 'scale(1)',
        }}
      />
      <div style={c.lg ? cruiseStyles.ovl : cruiseStyles.ovl} />
      <div
        style={{ ...cruiseStyles.copy, ...(c.lg ? cruiseStyles.copyLg : {}) }}
      >
        ...
        <div style={{ ...cruiseStyles.t, ...(c.lg ? cruiseStyles.tLg : {}) }}>
          ...
          <span style={cruiseStyles.em}>{c.em}</span>
        </div>
        {c.body && <p style={cruiseStyles.body}>{c.body}</p>}
        <div style={cruiseStyles.meta}>
          <span style={cruiseStyles.metaItem}>
  ...
  <section style={cruiseStyles.section} id="cruises">
    <div style={cruiseStyles.inner}>
      <div style={cruiseStyles.head}>
        ...
        <h2 style={cruiseStyles.h}>
          Choose your <ItalicEm>afternoon</ItalicEm>.
        </h2>
        <p style={cruiseStyles.sub}>
          ...
          Year&rsquo;s Eve on three decks.
        </p>
      </div>
      <div style={cruiseStyles.mosaic}>
        {CRUISES.map((c) => (
          <CruiseCard key={c.t + c.em} c={c} />
        ))}
      </div>
    </div>
  </section>
