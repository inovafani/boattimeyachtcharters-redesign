...
const CharterCard = ({ c }) => {
  ...
  return (
    <div
      style={charterStyles.card}
      onMouseEnter={() => setH(true)}
      onMouseLeave={() => setH(false)}
    >
      <div
        style={{
          ...charterStyles.img(c.img),
      ...
      <div style={charterStyles.copy}>
        ...
        <div style={charterStyles.t}>
          {c.t} <span style={charterStyles.em}>{c.em}</span>
        </div>
        <p style={charterStyles.body}>{c.body}</p>
        <a style={charterStyles.link}>
...
const Charters = () => (
  <section style={charterStyles.section}>
    <div style={charterStyles.head}>
      ...
      <h2 style={charterStyles.h}>
        Tailored for <ItalicEm>every occasion</ItalicEm>.
      </h2>
      <p style={charterStyles.sub}>
        From intimate sunset proposals to full-vessel corporate events — every
        charter is bespoke, every afternoon written to your brief.
      </p>
    </div>
    <div style={charterStyles.grid}>
      {CHARTERS.map((c) => (
        <CharterCard key={c.i} c={c} />
      ))}
    </div>
  </section>
);
