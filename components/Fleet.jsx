...
const fleetStyles = {
  ...
  card: { position: 'relative', aspectRatio: '4/5', overflow: 'hidden', cursor: 'pointer', background: 'var(--ocean)' },
  img: (url) => ({
    position: 'absolute', inset: 0,
    backgroundImage: `url(${url})`, backgroundSize: 'cover', backgroundPosition: 'center',
    transition: 'transform 0.8s cubic-bezier(.2,.6,.2,1)',
  }),
  ovl: { position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(10,22,40,0.95) 0%, rgba(10,22,40,0.35) 50%, rgba(10,22,40,0.1) 100%)' },
  copy: { position: 'absolute', left: 44, right: 44, bottom: 44 },
  e: { fontFamily: 'var(--font-body)', fontSize: 10, letterSpacing: '0.3em', textTransform: 'uppercase', color: 'var(--gold)', fontWeight: 500, marginBottom: 14 },
  t: { fontFamily: 'var(--font-display)', fontWeight: 400, fontSize: 54, color: 'var(--cream)', lineHeight: 1, marginBottom: 18, letterSpacing: '-0.015em' },
  em: { fontStyle: 'italic', color: 'var(--gold-light)' },
  body: { fontFamily: 'var(--font-body)', fontSize: 14, color: 'rgba(245,240,232,0.78)', lineHeight: 1.7, marginBottom: 28, maxWidth: 440 },
  specs: {
    display: 'grid', gridTemplateColumns: 'repeat(3, auto)', gap: 36,
    paddingTop: 22, borderTop: '1px solid rgba(201,168,76,0.28)',
    marginBottom: 28,
  },
  spec: { display: 'flex', flexDirection: 'column', gap: 6 },
  specKey: { fontFamily: 'var(--font-body)', fontSize: 9, letterSpacing: '0.28em', textTransform: 'uppercase', color: 'var(--gold)', fontWeight: 500 },
  specVal: { fontFamily: 'var(--font-display)', fontSize: 24, color: 'var(--cream)', fontWeight: 400 },
  link: {
...
const YachtCard = ({ y }) => {
  ...
  return (
    <div style={fleetStyles.card} onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}>
      <div style={{ ...fleetStyles.img(y.img), transform: h ? 'scale(1.06)' : 'scale(1)' }}/>
      <div style={fleetStyles.ovl}/>
      <div style={fleetStyles.copy}>
        <div style={fleetStyles.e}>{y.eye}</div>
        <div style={fleetStyles.t}>{y.name.split(' ')[0]} <span style={fleetStyles.em}>{y.name.split(' ').slice(1).join(' ')}</span></div>
        <p style={fleetStyles.body} dangerouslySetInnerHTML={{ __html: y.body }}/>
        ...
        <a style={fleetStyles.link}>Step aboard <Icon name="arrow" size={12} color="var(--gold)"/></a>
      </div>
    </div>
  );
};
  ...
  <section style={fleetStyles.section} id="fleet">
    <div style={fleetStyles.inner}>
      <div style={fleetStyles.head}>
        ...
        <h2 style={fleetStyles.h}>Two superyachts. <ItalicEm>One</ItalicEm> unhurried coastline.</h2>
        <p style={fleetStyles.sub}>Sun Goddess for head-turning arrivals. Mermaid Spirit for three decks of celebration. Both berthed at Marina Mirage, ready for your afternoon.</p>
      </div>
      <div style={fleetStyles.grid}>
        {YACHTS.map(y => <YachtCard key={y.name} y={y}/>)}
      </div>
    </div>
  </section>
);
